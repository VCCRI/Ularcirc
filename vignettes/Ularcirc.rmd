...
A number of Ularcirc functions are also available via the command line. 

## Loading data sets

Individual STAR chimeric read outputs can be loaded via the R command line. The function ```loadSTAR_chimeric``` loads STAR chimeric.out.juntion files (if gzipped must have .gz extension) generated from the STAR aligner. Note that the STAR aligner has undergone at least three major revisions in generating output for Chimeric reads and to accommodate all outputs the ```loadSTAR_chimeric``` returns a list of three items. The list item ```data_set`` contains a data table of the chimeric reads output, note that the number of columns returned is dependent on the number of columns requested (default is 21 columns). STAR aligner 2.7 generated 21 columns, 2.6 produced 15 columns, and STAR 2.5 and earlier version produced 14 columns.  

The list item ```alignmentStats``` and ```commandLine``` collect the meta data provided in STAR 2.7 outputs. These fields will be set to NULL if data was aligned with an earlier  STAR version.
...
The chimeric data can then be filtered to enrich for backsplice junctions. The default settings for ```FilterChimericJuncs``` will enrich most circRNAs in human and mouse genomes. This filtered list can then be collated into a ranked count list using the function ```SelectUniqueJunctions```.


```
chimericsDT$filtered <- Ularcirc::FilterChimericJuncs(chimericsDT$data_set, canonicalJuncs = TRUE)

filterlist <- list(BSjuncName=NULL, SortDir="Descending", IndexNumber=1, DisplayNumber=50, DisplayRAD_score= FALSE, Apply_FSJ_Filter=FALSE, unstranded=TRUE)

# The following will assemble a tally of counts for each potential backsplice junction. Note that this data is unstranded so we need to set the unstranded flag.
chimericsDT$summary  <- Ularcirc::SelectUniqueJunctions(chimericsDT$filtered, filterlist = filterlist, unstranded=TRUE)
```
...
Temporary files are created and used throughout a Ularcirc session. For large data sets this is likely to create large temporary files which could impact on file allocation resources. This might be problematic for unix systems as the default cache locations are shown below (taken from rappdirs):  
...
source https://www.rdocumentation.org/packages/rappdirs/versions/0.3.1/topics/user_cache_dir



Note that unix systems can actually define the location by setting/changing the XDG_CACHE_HOME environment variable. This can be done by typing the following line in your .profile file (permanent change) or on the command line (remains active for life of session).

```
# In the following code  modify /scratch/tmp/ to a location that exists on your system
mkdir /scratch/tmp/BiocFileCache
